...
//Predicting Adversary Tie Response In Combat Intelligently and Automatically
...
class Bot {
    ...
    makeMove(gamestate: Gamestate): BotSelection {
        ...
        if (l == 5 && !winner && gamestate.rounds[0].p2 === 'R' && gamestate.rounds[1].p2 === 'P' && gamestate.rounds[2].p2 === 'S' && gamestate.rounds[3].p2 === 'R' && gamestate.rounds[4].p2 === 'R') {
            this.losing = true;
        }
        if (this.losing) {
            return 'W';
        ...
        const oppo_has_dyn = (this.dyns[1]>0);
        // If oppo plays dynamite:
        const oppo_dyn_prob = oppo_has_dyn ? oppo_plays['D'] / total_oppo_plays : 0;
        other_val -= oppo_dyn_prob;
        water_val += oppo_dyn_prob;

        // If oppo plays water:
        const oppo_water_prob = oppo_has_dyn ? oppo_plays['W'] / total_oppo_plays : oppo_plays['W'] / (oppo_plays['W'] + oppo_plays['O']);
        dyn_val -= oppo_water_prob;
        other_val += oppo_water_prob;

        const oppo_other_prob = oppo_has_dyn ? oppo_plays['O'] / total_oppo_plays : oppo_plays['O'] / (oppo_plays['W'] + oppo_plays['O']);
